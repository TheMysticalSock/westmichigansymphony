...
function custevnt_func(){
	...
	echo '<ul class="list-grid-style">
		...
		<li><a class="list-view" href="'.site_url().'/concerts-tickets/concert-calendar/?monthadd='.date('m').'&yearadd='.date('Y').'"><img src="'.plugins_url( 'images/list-view.png', __FILE__ ).'" /></a></li>
		<li><a class="grid-view" href="'.site_url().'/concerts-tickets/concert-calendar/?typ=full&monthadd=1&yearadd='.date('Y').'"><img src="'.plugins_url( 'images/grid-view.png', __FILE__ ).'" /></a></li>
	</ul>';
...
function custblock_func(){
	...
	echo '<ul class="list-grid-style">
		...
		<li><a class="grid-view" href="'.site_url().'/the-block/concert-calendar/?typ=full&monthadd=1&yearadd='.date('Y').'"><img src="'.plugins_url( 'images/grid-view.png', __FILE__ ).'" /></a></li>
	</ul>';
	echo "<form name='taax1' id='taax1' action='' method='post' class='taxonomy_form'>";
	echo get_cat_frm_taxonomy1('concerttypes');
	echo get_cat_frm_taxonomy2('composers');
	...
	$val = "The Block";
	$taxonomysearch1 = ($_POST['tax1']!='' ? $_POST['tax1'] : $val);
	$taxonomysearch2 = ($_POST['tax2']!='' ? $_POST['tax2'] : '');
...
function ShowConcertevent_func()
{
	...
	echo '<div class="event_panel">
				<ul class="event">';
	$eventsarrs = $wpdb->get_results( "SELECT t.* FROM wp_eventdate t,wp_posts y WHERE y.ID=t.eventid and y.post_status='publish' ORDER by t.eventstartdate ASC");
	foreach ($eventsarrs as $eventsarr)
	{
		...
		echo '<li>
				<div class="concertdate">
					<p>'.date('l', strtotime( $tmro)).'</p>
					<p>'.date('F', strtotime( $tmro)).'</p>
					<p class="number">'.date('d', strtotime( $tmro)).'</p>
					<p class="time">'.$event_start_time.'</p>
				</div>
				...
				<div class="panel_content">
					<h2> '.get_the_title($eventsarrs->eventid).'
						<span class="buy">'.$buy_link.'</span>
					</h2>
					<div class="cont_mid">
						<p>'.substr($eventsarr->eventcategory,4).'</p>
					</div>
					<p>
						'.$string2.'
						<span>
...
function top3event_func(){
	...
	$eventsarrs = $wpdb->get_results( "SELECT t.* FROM wp_eventdate t,wp_posts y WHERE y.ID=t.eventid and y.post_status='publish' ORDER by t.eventstartdate ASC LIMIT 0,3");
	foreach ($eventsarrs as $eventsarr)
	{
		$image = wp_get_attachment_image_src( get_post_thumbnail_id( $eventsarr->eventid ));
	...
	echo '<li>
			<img src="'.$image[0].'" alt="" />
			<h2>'.$eventsarr->eventstartdate .' '. $eventsarr->eventstarttime.'<br></h2>
			<p>'.$string2.'</p>
			<a href="'.site_url().'/eventdetails?evnid='.$eventsarr->id.'" style="color:#00A6BD!important;"> MORE </a>
			<div class="bn">
				'.$buy_link.'
