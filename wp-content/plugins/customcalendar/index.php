<?php
/*
 Plugin Name: Custom Event Calendar Plugin
Plugin URI: http://www.capitalnumber.com
Description: You Can Create Event Using this
Version: The Plugin's Version Number, e.g.: 1.0
Author: Souvik Chaudhury
Author URI: http://www.capitalnumber.com
...
function custevnt_func(){
	...
	if($_GET['typ']=='' && $_GET['monthadd']==''){
		...
		echo '<ul class="month-list-view">
					<li><a href="'.site_url().'/concerts-tickets/concert-calendar/?cal=yes&monthadd='.$mmon.'&yearadd='.$years.'">'.date('F',mktime(0,0,0,$mmon,1,$years)).' '.$years.'</a>
						<ul>';
		...
		for($i=0;$i<12;$i++){
			...
			echo '<li><a href="'.site_url().'/concerts-tickets/concert-calendar/?cal=yes&monthadd='.$mmon.'&yearadd='.$years.'">'.date('F',mktime(0,0,0,$mmon,1,$yrs)).' '.$years.'</a></li>';
	...
	if ($_GET['monthadd']!='' && $_GET['yearadd']!='' && $_GET['typ']=='')
	{
		//date_funct_button();
		$mmon = $_GET['monthadd'];
		$years = $_GET['yearadd'];
		...
		echo '<ul class="month-list-view">
					<li><a href="'.site_url().'/concerts-tickets/concert-calendar/?monthadd='.$_GET['monthadd'].'&yearadd='.$_GET['yearadd'].'">'.date('F',mktime(0,0,0,$_GET['monthadd'],1,$_GET['yearadd'])).' '.$_GET['yearadd'].'</a>
						<ul>';
		for($i=0;$i<12;$i++){
			if($i==0) {
				if($mmon == 1){
...
function ShowConcertevent_func()
{
	...
	foreach ($eventsarrs as $eventsarr)
	{
		...
		if(strtotime($eventsarr->eventstartdate) < mktime(0,0,0,date('m'),date('d'),date('Y'))){
			$buy_link = '';
		}
	...
	echo "</pre>";*/
		...
		echo '<li>
				...
				<div class="pic_panel">
					<a title="View Event" href="'.site_url().'/eventdetails?evnid='.$eventsarr->id.'"><img src="'.$image[0].'" alt="" /></a>
				</div>
				<div class="panel_content">
					<h2><a href="'.site_url().'/eventdetails?evnid='.$eventsarr->id.'" title="View Event">'.get_the_title($eventsarr->eventid).'</a></h2>
					...
					<p>
						'.$string2.'
						<span>
						  <a class="program-more-link" href="'.site_url().'/eventdetails?evnid='.$eventsarr->id.'" style="color:#00A6BD!important;" title="View Event"> MORE&nbsp;></a>
						</span>
					</p>
				</div>
...
function ShowConcerteventblock_func(){
	...
	foreach ($eventsarrs as $eventsarr)
	{
		...
		echo '<li>
				...
				<div class="color" style="background:'.$optname.'"> </div>
				<div class="pic_panel">
					<a href="'.site_url().'/eventdetails?evnid='.$eventsarr->id.'" title="View Event"><img src="'.$image[0].'" alt="" /></a>
				</div>
				<div class="panel_content">
					<h2><a href="'.site_url().'/eventdetails?evnid='.$eventsarr->id.'" title="View Event">'.get_the_title($eventsarr->eventid).'</a></h2>
					...
					<p>'.$string2.'
						<span>
						  <a href="'.site_url().'/eventdetails?evnid='.$eventsarr->id.'" style="color:#00A6BD!important;" title="View Event"> MORE&nbsp;></a>
						</span>
					</p>
				</div>
				<div class="clear"></div>
...
function top3event_func(){
	...
	$esd = date('Y/m/d', mktime(0,0,0,date('m'),date('d'),date('Y')));
	$eventsarrs = $wpdb->get_results( "SELECT t.id, t.eventid, min(t.eventstartdate) as eventstartdate, max(t.eventstartdate) as eventenddate, t.eventstarttime, t.eventendtime, t.eventcategory FROM wp_eventdate t,wp_posts y WHERE y.ID=t.eventid and y.post_status='publish' and t.eventstartdate >= '".$esd."' group by t.eventid ORDER by t.eventstartdate ASC LIMIT 0,3");
	foreach ($eventsarrs as $eventsarr)
	{
		$image = wp_get_attachment_image_src( get_post_thumbnail_id( $eventsarr->eventid ));
		...
		if($buy_button[0]!='Disable'){
			if($eventsarr->eventstartdate != $eventsarr->eventenddate){
				// start date and end date are not the same so there are two buy ticket buttons here
				$buy_link = '
				<div class="bn"><a class="buy-ticket-2" href="'.$link[0].'" target="_blank">Buy '.date("l", strtotime($eventsarr->eventstartdate)).' Tickets</a>
				<a class="buy-ticket-2" href="'.$link[0].'-2" target="_blank">Buy '.date("l", strtotime($eventsarr->eventenddate)).' Tickets</a></div>';
			} else {
				$buy_link = '<div class="bn"><a class="buy-ticket" href="'.$link[0].'" target="_blank">Buy Tickets</a></div>';
			}
		}
...
function sponsor_main()
{
	...
	<p>You Can Add and Delete Various types of Sponsor images and Link (Media , Concert, Guest Artist)</p>
	";
}
...
/* =================================================================================================
						 Add Sub Menu in Admin Panel for custom post type
====================================================================================================*/
function Guestartistsponsor1()
{
	require('guestartistsponsor.php');
}

add_action("admin_menu","add_admin_guestartistsponsor");
function add_admin_guestartistsponsor()
{
	add_submenu_page( "sponsor", "Guest Artist Sponsor", "Guest Artist Sponsor", "manage_options", "guestartistsponsor", Guestartistsponsor1 );
	//add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function );
}
/* =================================================================================================
 							Add Sub Menu in Admin Panel for custom post type
