...
$theID = get_the_ID();
	...
	$totalRows_get_sponsor = mysql_num_rows($get_sponsor);
        ...
        mysql_select_db($database_x_wsso, $x_wsso);
	$query_get_media_sponsor = "SELECT * FROM x_wp_extra_media_sponsor WHERE ex_id = $theID";
	$get_media_sponsor = mysql_query($query_get_media_sponsor, $x_wsso) or die(mysql_error());
...
<a href="/staging/calendar/">< TO THE SCHEDULE <?=$xDate  ?></a>
  ...
  <div class="event-page-right">
    <div class="event-page-image"><img src="/staging/extra_images/<?php echo $row_get_conductor['conductor_image']; ?>" alt="<?php echo $row_get_conductor['conductor']; ?>">
    <span><?php echo $row_get_conductor['conductor']; ?></span> </div>
    
      <div class="event-page-sponsors">
  <?php if($totalRows_get_sponsor!=0){?><p class="sponsor-p">Concert Sponsor:</p><?php } ?>
  ...
  <?php do { ?>
    <a href="<?php echo $row_get_sponsor['sponsor_url']; ?>"><img src="/staging/extra_images/<?php echo $row_get_sponsor['sponsor_image']; ?>" alt="<?=$row_get_sponsor['sponsor_name'] ?>"></a>
       ...
       <div class="event-page-sponsors">
  <?php if($totalRows_get_media_sponsor!=0){?><p class="sponsor-p">Media Sponsor:</p><?php } ?>
   
  <?php do { ?>
    <a href="<?php echo $row_get_media_sponsor['sponsor_url']; ?>"><img src="/staging/extra_images/<?php echo $row_get_media_sponsor['sponsor_image']; ?>" alt="<?=$row_get_sponsor['sponsor_name'] ?>"></a>
