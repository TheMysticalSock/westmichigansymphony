		...
		if(!empty($taxonomies)){	
			...
			foreach($taxonomies as $taxonomy => $tax){
				...
				if(is_array($terms) && count($terms)>0){
					...
					$tmp.='<div class="fbd-unchecked"><div rel="%s" class="fbd-cell"><select class="fbd-checkbox fbd-filter">';
					foreach($terms as $i => $term){
						//echo "<option name='hi'>".$term->slug."</option>";
						
						$tmp.=sprintf('<option name="'.$term->slug.'">'.$term->name.'</option>',
							$i,
